...
object RDD_Operations {
  def main(args: Array[String]): Unit = {
    ...
    // --- Transformation 4: groupByKey ---
    println("\n--- Transformation 4: groupByKey (Language Count) ---")
    println("Grouping language occurrences and summing counts per language")

    val langGroups = languages
      .map(lang => (lang, 1))
      .groupByKey()
      .map(x => (x._1, x._2.sum))
    ...
    println("-" * 32)
    langGroups.take(10).foreach(x => println(f"${x._1}%-20s ${x._2}%10d"))
